...
// calls get the same resilience). An explicit VITE_VJAIPUR_WORKER_URL
// disables the fallback entirely — the same-origin proxy targets PROD, which
// would be the wrong backend for a staging override. The WS nudge stays
// direct: a blocked WS just degrades to the sync polling that is the data
// path anyway.
const PROXY_BASE = '/api'

const proxyFallback = createProxyFallbackFetcher({
  directBase: workerBaseUrl,
  proxyBase: () => {
...
export const __setProxyBaseForTests = proxyFallback.setProxyBaseForTests
...
async function rawFetch(path: string, method: string, body: unknown, token: string | null | undefined): Promise<Response> {
  ...
  return proxyFallback.fetchWithFallback(path, init)
}
